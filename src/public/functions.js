...
const getWeatherConditions =async(city) => {
    await fetch(`http://api.weatherstack.com/current?access_key=74a3366fca697f220438019c416500ed&query=${city}`) ////(`http://api.openweathermap.org/data/2.5/weather?q=${city}&appid=5a2df598fe17541c11256f4f06d77310`)
    .then(res => res.json())
    .then(data => {
        console.log(data)
        ...
        let weatherDescNode = document.createTextNode(data.current.weather_descriptions[0]); //current.weather_descriptions
        ...
        let tempNode = document.createTextNode(data.current.temperature + "  °Celcius");
